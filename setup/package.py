...
exec(open(os.path.join(ROOTDIR, 'galex', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/galex && \
    groupadd -g %(group_id)s galex && \
    useradd -u %(user_id)s -g galex galex -d /var/lib/galex && \
    mkdir /data && \
    chown galex:galex /var/lib/galex /data
USER galex
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class GalexERPTestTimeoutError(Exception):
    pass


class GalexERPTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise GalexERPTestError("Installation of package failed")
        else:
        ...
        raise GalexERPTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/galex_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.galex_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'galex/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'galex-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='galex', exposed_port=None, detach=False, timeout=None):
    ...
    def test_galex(self):
        logging.info('Starting to test GalexERP install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'galex.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise GalexERPTestTimeoutError('GalexERP pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise GalexERPTestError('Error while installing/starting GalexERP after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'galex-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/galex-*.tar.gz' % self.args.build_dir)[0], '%s/galex_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/galex-*.zip' % self.args.build_dir)[0], '%s/galex_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/galex_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s galex"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su galex -s /bin/bash -c "galex -d mycompany -i base --stop-after-init"',
            'su galex -s /bin/bash -c "galex -d mycompany --pidfile=/data/src/galex.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'galex-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_galex()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/galex (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../galex_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'galex-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/galex_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su galex -s /bin/bash -c "galex -d mycompany -i base --stop-after-init"',
            'su galex -s /bin/bash -c "galex -d mycompany --pidfile=/data/src/galex.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'galex-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_galex()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'galex-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/galex-*.noarch.rpm' % self.args.build_dir)[0], '%s/galex_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/galex_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su galex -s /bin/bash -c "galex -c /etc/galex/galex.conf -d mycompany -i base --stop-after-init"',
            'su galex -s /bin/bash -c "galex -c /etc/galex/galex.conf -d mycompany --pidfile=/data/src/galex.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'galex-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_galex()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'galex-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/galexbuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing GalexERP files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/galex_setup_*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('galex_setup_')[1].split('.exe')[0]

        self.rsync(['%s' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"GalexERP %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"GalexERP {sv}"/python/python.exe \'c:\\Program Files\\GalexERP {sv}\\server\\galex-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/galex/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/galex/vm/win1036/id_rsa', help="%(default)s")

...
!ifndef SERVICENAME
	!define SERVICENAME 'galex-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\galexbuild'
!endif

!define PRODUCT_NAME "GalexERP"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\GalexERP Server ${VERSION}"
...
OutFile "galex_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\galex-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\galex-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\galex-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\galex-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.galex.com/page/contactus"
...
; English
LangString DESC_GalexERP_Server ${LANG_ENGLISH} "Install the GalexERP Server with all the GalexERP standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by GalexERP."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact GalexERP for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the GalexERP Server"
...
LangString TITLE_GalexERP_Server ${LANG_ENGLISH} "GalexERP Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start GalexERP"

; French
LangString DESC_GalexERP_Server ${LANG_FRENCH} "Installation du Serveur GalexERP avec tous les modules GalexERP standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par GalexERP."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez GalexERP pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur GalexERP"
...
LangString TITLE_GalexERP_Server ${LANG_FRENCH} "Serveur GalexERP"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer GalexERP"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_GalexERP_Server) SectionGalexERP_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the GalexERP Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "addons_path" "$INSTDIR\server\galex\addons"
    WriteIniStr "$INSTDIR\server\galex.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\galex.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\galex-bin" --stop-after-init --logfile "$INSTDIR\server\galex.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\galex-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_GalexERP" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@galex.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.galex.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@galex.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionGalexERP_Server} $(DESC_GalexERP_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        ${If} ${RunningX64}
            StrCpy $INSTDIR "$PROGRAMFILES64\GalexERP ${VERSION}"
        ${Else}
            StrCpy $INSTDIR "$PROGRAMFILES\GalexERP ${VERSION}"
        ${EndIf}
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionGalexERP_Server} $0

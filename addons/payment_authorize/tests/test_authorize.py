...
from galex.addons.payment.tests.common import PaymentAcquirerCommon
from galex.addons.payment_authorize.controllers.main import AuthorizeController
...
@galex.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('galex.addons.payment_authorize.models.payment', 'ValidationError')
...
@galex.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on galex/galex + create sandbox account
        authorize.write({

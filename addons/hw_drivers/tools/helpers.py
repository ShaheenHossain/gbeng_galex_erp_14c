...
class IoTRestart(Thread):
    """
    Thread to restart galex server in IoT Box when we must return a answer before
    """
...
def add_credential(db_uuid, enterprise_code):
    write_file('galex-db-uuid.conf', db_uuid)
    write_file('galex-enterprise-code.conf', enterprise_code)

def check_certificate():
    """
    Check if the current certificate is up to date or not authenticated
    """
    server = get_galex_server_url()
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'GalexERPTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected GalexERP DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/galex/', '--git-dir=/home/pi/galex/.git']
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/galex/addons/hw_drivers/iot_handlers/drivers/*"])
                    subprocess.check_call(["rm", "-rf", "/home/pi/galex/addons/hw_drivers/iot_handlers/interfaces/*"])
...
def check_image():
    ...
    """
    url = 'https://nightly.galex.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_galex_server_url():
    ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
    if not ap:
        return False
    return read_file_first_line('galex-remote-server.conf')

def get_token():
    return read_file_first_line('token')

def get_version():
    return subprocess.check_output(['cat', '/var/galex/iotbox_version']).decode().rstrip()
...
def load_certificate():
    """
    Send a request to GalexERP with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('galex-db-uuid.conf')
    enterprise_code = read_file_first_line('galex-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.galex.com/galex-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('galex-subject.conf', result['subject_cn'])
            subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured GalexERP server
    """
    server = get_galex_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'galex/addons/hw_drivers/iot_handlers'
...
def load_iot_handlers():
    """
    This method loads local files: 'galex/addons/hw_drivers/iot_handlers/drivers' and
    'galex/addons/hw_drivers/iot_handlers/interfaces'

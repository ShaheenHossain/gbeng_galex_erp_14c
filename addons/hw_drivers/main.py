...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to GalexERP database
        """
        server = helpers.get_galex_server_url()
        if server:
            subject = helpers.read_file_first_line('galex-subject.conf')
            if subject:
            ...
            _logger.warning('GalexERP server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the galex server with the updates
        """

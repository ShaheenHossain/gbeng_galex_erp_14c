...
from galex import http, _
from galex.addons.hw_drivers.controllers.proxy import proxy_drivers
from galex.addons.hw_drivers.driver import Driver
from galex.addons.hw_drivers.event_manager import event_manager
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = Path.home() / 'galex-keyboard-layouts.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('galex-keyboard-layouts.conf', json.dumps(data))
    ...
    def save_is_scanner(self, is_scanner):
        ...
        """
        file_path = Path.home() / 'galex-keyboard-is-scanner.conf'
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = is_scanner
        helpers.write_file('galex-keyboard-is-scanner.conf', json.dumps(data))
    ...
    def _keyboard_input(self, scancode):
        ...
        pressed key represented by its scancode to the connected GalexERP instance.

...
from galex.addons.google_account.models.google_service import GoogleService
from galex.addons.google_calendar.models.res_users import User
from galex.addons.google_calendar.models.google_sync import GoogleSync
from galex.modules.registry import Registry
from galex.addons.google_account.models.google_service import TIMEOUT
from galex.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api


@patch.object(User, '_get_google_calendar_token', lambda user: 'dummy-token')
class TestSyncGalexERP2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_galex2google(self.google_service)
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'galexbot@example.com', 'self': True},
            'attendees': [{'email': 'jean-luc@opoo.com', 'responseStatus': 'needsAction'}],
            'extendedProperties': {'shared': {'%s_galex_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_galex2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_galex_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_galex_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
    ...
    def test_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=True)._sync_galex2google(self.google_service)
    ...
    def test_not_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=False)._sync_galex2google(self.google_service)
        self.call_post_commit_hooks()

...
from galex.addons.google_calendar.utils.google_calendar import GoogleEvent
import pytz
from datetime import datetime, date
from dateutil.relativedelta import relativedelta
from galex.tests.common import new_test_user
from galex.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api


class TestSyncGoogle2GalexERP(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2galex(google_recurrence)
        self.env['calendar.event']._sync_google2galex(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'galexcalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2galex(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'galexbot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_galex_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2galex(GoogleEvent([values]))
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2galex(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])

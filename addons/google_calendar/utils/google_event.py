...
from galex.tools.sql import existing_tables
import pytz
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by GalexERP recordset, one instance can be a single Google event or a
    ...
    def galex_id(self, env):
        self.galex_ids(env)  # load ids
        return self._galex_id

    def _meta_galex_id(self, dbname):
        """Returns the GalexERP id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_galex_id' % dbname)
        if o_id:
            return int(o_id)

    def galex_ids(self, env):
        ids = tuple(e._galex_id for e in self if e._galex_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_galex_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_galex_ids_from_metadata(env, model)

        return tuple(e._galex_id for e in self)

    def _load_galex_ids_from_metadata(self, env, model):
        unsure_galex_ids = tuple(e._meta_galex_id(env.cr.dbname) for e in self)
        galex_events = model.browse(_id for _id in unsure_galex_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same GalexERP id.
        # Therefore, we only consider GalexERP records without google id when trying
        # to match events.
        o_ids = galex_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            galex_id = e._meta_galex_id(env.cr.dbname)
            if galex_id in o_ids:
                e._events[e.id]['_galex_id'] = galex_id

    def _load_galex_ids_from_db(self, env, model):
        galex_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in galex_events}  # {google_id: galex_id}
        existing_google_ids = galex_events.mapped('google_id')
        for e in self:
            galex_id = mapping.get(e.id)
            if galex_id:
                e._events[e.id]['_galex_id'] = galex_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and GalexERP.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in GalexERP (he is the owner) but userB syncs first.
        ...
        # (he is the organizer in Google). The "real" owner (in GalexERP) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in GalexERP several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_galex_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.galex_ids(env)
        events.galex_ids(env)

        return self.filter(lambda e: e._galex_id)

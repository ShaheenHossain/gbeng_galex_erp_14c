...
from galex import api, fields, models
...
class RecurrenceRule(models.Model):
    ...
    def _write_from_google(self, gevent, vals):
        ...
        # event_tz is written on event in Google but on recurrence in GalexERP
        vals['event_tz'] = gevent.start.get('timeZone')
        super()._write_from_google(gevent, vals)

        base_event_time_fields = ['start', 'stop', 'allday']
        new_event_values = self.env["calendar.event"]._galex_values(gevent)
    ...
    def _create_from_google(self, gevents, vals_list):
        for gevent, vals in zip(gevents, vals_list):
            base_values = dict(
                self.env['calendar.event']._galex_values(gevent),  # FIXME default reminders
                need_sync=False,
            )
            # If we convert a single event into a recurrency on Google, we should reuse this event on GalexERP
    ...
    @api.model
    def _galex_values(self, google_recurrence, default_reminders=()):
        return {

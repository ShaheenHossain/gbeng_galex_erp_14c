...
from galex import api, fields, models, registry, _
from galex.tools import ormcache_context
from galex.exceptions import UserError
from galex.osv import expression

from galex.addons.google_calendar.utils.google_event import GoogleEvent
from galex.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from galex.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the GalexERP database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in GalexERP.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on GalexERP DB (_sync_google2galex).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_galex2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2galex(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in GalexERP. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in GalexERP
        :return: synchronized galex recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        galex_values = [
            dict(self._galex_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_galex = self.with_context(dont_notify=True)._create_from_google(new, galex_values)
        ...
        if not self._context.get("dont_notify") and all(not e.is_recurrence() for e in google_events):
            new_galex._notify_attendees()

        cancelled = existing.cancelled()
        cancelled_galex = self.browse(cancelled.galex_ids(self.env))
        cancelled_galex._cancel()
        synced_records = (new_galex + cancelled_galex).with_context(dont_notify=self._context.get("dont_notify", False))
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and galex server time are different
            updated = parse(gevent.updated)
            galex_record = self.browse(gevent.galex_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not galex_record.write_date or updated >= pytz.utc.localize(galex_record.write_date):
                vals = dict(self._galex_values(gevent, default_reminders), need_sync=False)
                galex_record._write_from_google(gevent, vals)
                synced_records |= galex_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from GalexERP to Google
    ...
    @api.model
    def _galex_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _notify_attendees(self):
        ...
        This is called when creating new calendar events in _sync_google2galex.
        At the initialization of a synced calendar, GalexERP requests all events for a specific

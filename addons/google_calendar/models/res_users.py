...
import logging
import requests
from galex.addons.google_calendar.models.google_sync import google_calendar_token
from datetime import timedelta


from galex import api, fields, models, _
from galex.exceptions import UserError
from galex.loglevels import exception_to_unicode
from galex.addons.google_account.models.google_service import GOOGLE_TOKEN_ENDPOINT
from galex.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
...
class User(models.Model):
    ...
    google_calendar_cal_id = fields.Char('Calendar ID', copy=False, help='Last Calendar ID who has been synchronized. If it is changed, we remove all links between GoogleID and GalexERP Google Internal ID')
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2galex(recurrences)
        synced_events = self.env['calendar.event']._sync_google2galex(events - recurrences, default_reminders=default_reminders)

        # GalexERP -> Google
        send_updates = not full_sync
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_galex2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_galex2google(calendar_service)

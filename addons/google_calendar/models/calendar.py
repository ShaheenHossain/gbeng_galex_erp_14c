...
from galex import api, fields, models, tools, _
...
class Meeting(models.Model):
    ...
    @api.model
    def _galex_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._galex_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._galex_attendee_commands(google_event)
        values = {
    ...
    @api.model
    def _galex_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.galex_id(self.env)).attendee_ids
        ...
        for galex_attendee in attendees_by_emails.values():
            ...
            if tools.email_normalize(galex_attendee.email) not in emails:
                attendee_commands += [(2, galex_attendee.id)]
                partner_commands += [(3, galex_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _galex_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_galex_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user:
            ...
            # If several galex users are attendees but the owner is not in galex, the event will be duplicated on galex database
            ...
            # values['extendedProperties']['private] should be used if the owner is not an galex user

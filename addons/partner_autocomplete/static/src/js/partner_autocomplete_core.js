galex.define('partner.autocomplete.Mixin', function (require) {
'use strict';
...
var PartnerAutocompleteMixin = {
    _dropPreviousGalexERP: new concurrency.DropPrevious(),
    ...
    _autocomplete: function (value) {
        ...
        var galexSuggestions = [];
        var clearbitSuggestions = [];
        return new Promise(function (resolve, reject) {
            var galexPromise = self._getGalexERPSuggestions(value, isVAT).then(function (suggestions){
                galexSuggestions = suggestions;
            });
            ...
            var concatResults = function () {
                // Add Clearbit result with GalexERP result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    var websites = galexSuggestions.map(function (suggestion) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            galexSuggestions.push(suggestion);
                        }
                    });
                }

                galexSuggestions = _.filter(galexSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(galexSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(galexSuggestions);
            };

            self._whenAll([galexPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    ...
    /**
     * Use GalexERP Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
    ...
    _getGalexERPSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousGalexERP.add(def);
    },
    /**

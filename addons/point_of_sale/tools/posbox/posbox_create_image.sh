...
REPO=https://github.com/galex/galex.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/galex"
...
galex/
galex-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/galex
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/galex/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version

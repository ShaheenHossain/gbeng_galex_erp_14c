...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/galex" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/galex/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/galex/* /home/pi/galex/
rm -r /home/pi/galex/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service galex restart) &

...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/galex/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/galex/addons/point_of_sale/tools/posbox/configuration/nginx_ap.conf

		service galex restart
	fi
# wired
else
	killall nginx

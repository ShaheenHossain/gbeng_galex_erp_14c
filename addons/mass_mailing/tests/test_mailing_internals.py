...
from galex.addons.mass_mailing.tests.common import MassMailCommon
from galex.tests.common import users, Form
from galex.tools import formataddr, mute_logger
...
class TestMassMailFeatures(MassMailCommon):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
% set url = "www.galex.com"
% set httpurl = "https://www.galex.eu"
Website0: <a id="url0" href="https://www.galex.tz/my/${object.name}">https://www.galex.tz/my/${object.name}</a>
Website1: <a id="url1" href="https://www.galex.be">https://www.galex.be</a>
...
Email: <a id="url5" href="mailto:test@galex.com">test@galex.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'email',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.galex.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.galex.be', True),
                              ('url2', 'https://www.galex.com', True),
                              ('url3', 'https://www.galex.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@galex.com', False)]:

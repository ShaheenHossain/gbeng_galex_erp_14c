...
from galex.addons.base.tests.common import TransactionCaseWithUserDemo, HttpCaseWithUserPortal
from galex.addons.website.tools import MockRequest
from galex.tests import tagged
from galex.tests.common import HttpCase, TransactionCase
from galex.tools import DotDict
...
class TestWebsitePriceList(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('galex.addons.website_sale.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
        patcher.start()
...
def simulate_frontend_context(self, website_id=1):
    ...
    patcher = patch('galex.addons.website.models.ir_http.get_request_website', wraps=get_request_website)
    patcher.start()
    self.addCleanup(patcher.stop)

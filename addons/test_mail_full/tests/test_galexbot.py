...
from galex.addons.test_mail.tests.common import TestMailCommon, TestRecipients
from galex.tests import tagged
from galex.tools import mute_logger


@tagged("galexbot")
class TestGalexERPbot(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestGalexERPbot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.galexbot = cls.env.ref("base.partner_root")
        ...
        cls.galexbot_ping_body = '<a href="http://galex.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@GalexERPBot</a>' % (cls.galexbot.id, cls.galexbot.id)
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('galex.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.env['mail.channel'].with_user(self.user_employee).init_galexbot()
        partners = self.env['mail.channel'].channel_fetch_listeners(channel.uuid)
        galexbot = self.env.ref("base.partner_root")
        galexbot_in_fetch_listeners = [partner for partner in partners if partner['id'] == galexbot.id]
        self.assertEqual(len(galexbot_in_fetch_listeners), 1, 'galexbot should appear only once in channel_fetch_listeners')

    @mute_logger('galex.addons.mail.models.mail_mail')
    def test_galexbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.galexbot_ping_body, 'partner_ids': [self.galexbot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.galexbot,
                answer=False
            )
        # GalexERPbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.galexbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last galexbot message instead
            sender=self.galexbot,
            answer=("@GalexERPBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.galexbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.galexbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('galex.addons.mail.models.mail_mail')
    def test_galexbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @galexbot right now", 'partner_ids': []})
        self.assertNextMessage(

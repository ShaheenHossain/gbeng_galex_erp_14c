...
from galex.addons.resource.models.resource import float_to_time
from galex.tools import is_html_empty
from galex.tools.translate import html_translate
    ...
    def _onchange_is_exhibitor(self):
        ...
        for sponsor in self:
            if sponsor.is_exhibitor and not sponsor.room_name:
                if sponsor.name:
                    room_name = "galex-exhibitor-%s" % sponsor.name
                else:
    ...
    def create(self, values_list):
        for values in values_list:
            if values.get('is_exhibitor') and not values.get('room_name'):
                ...
                name = 'galex-exhibitor-%s' % exhibitor_name or 'sponsor'
    ...
    def write(self, values):
        ...
        if values.get('is_exhibitor') and not values.get('chat_room_id') and not values.get('room_name'):
            ...
            for exhibitor in toupdate:
                values['room_name'] = 'galex-exhibitor-%s' % exhibitor.name

...
import galex.addons.iap.tools.iap_tools
from galex import http, tools
from galex.http import request
from galex.tools.misc import formatLang
...
class MailClientExtensionController(http.Controller):
    ...
    def auth(self, **values):
        """
         Once authenticated this route renders the view that shows an app wants to access GalexERP.
    ...
    def auth_confirm(self, scope, friendlyname, redirect, info=None, do=None, **kw):
        """
        Called by the `app_auth` template. If the user decided to allow the app to access GalexERP, a temporary auth code
    ...
    def auth_access_token(self, auth_code, **kw):
        ...
        scope = 'galex.plugin.' + auth_message.get('scope', '')
    ...
    def _get_auth_code_data(self, auth_code):
        ...
        signature = galex.tools.misc.hmac(request.env(su=True), 'mail_client_extension', data).encode()
    ...
    def _generate_auth_code(self, scope, name):
        ...
        signature = galex.tools.misc.hmac(request.env(su=True), 'mail_client_extension', auth_message).encode()
    ...
    def _iap_enrich(self, domain):
        ...
        #except galex.addons.iap.models.iap.InsufficientCreditError as ice:
        except galex.addons.iap.tools.iap_tools.InsufficientCreditError:

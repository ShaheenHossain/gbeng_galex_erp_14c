...
from galex.tools.float_utils import float_round
from galex.tools import consteq
from galex.exceptions import ValidationError

from galex.addons.payment_stripe.controllers.main import StripeController
...
class PaymentAcquirerStripe(models.Model):
    ...
    stripe_webhook_secret = fields.Char(
        ...
        help="If you enable webhooks, this secret is used to verify the electronic "
             "signature of events sent by Stripe to GalexERP. Failing to set this field in GalexERP "
    ...
    def _verify_stripe_signature(self):
        ...
        if not consteq(expected_signature, actual_signature):
            _logger.error(
                ...
                'in GalexERP matches to one in the Stripe dashboard')
    ...
    def _handle_checkout_webhook(self, checkout_object: dir):
        ...
        try:
            galex_tx = self.env['payment.transaction']._stripe_form_get_tx_from_data(data)
        except ValidationError as e:
            _logger.info('Received notification for tx %s. Skipped it because of %s', tx_reference, e)
            return False

        PaymentAcquirerStripe._verify_stripe_signature(galex_tx.acquirer_id)

        url = 'payment_intents/%s' % galex_tx.stripe_payment_intent
        stripe_tx = galex_tx.acquirer_id._stripe_request(url)

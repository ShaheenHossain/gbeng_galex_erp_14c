...
from galex import exceptions
from galex.addons.mail.tests.common import mail_new_test_user
from galex.addons.test_mail.data import test_mail_data
from galex.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE
from galex.addons.test_mail.models.test_mail_models import MailTestGateway
from galex.addons.test_mail.tests.common import TestMailCommon
from galex.tests import tagged
from galex.tests.common import users
from galex.tools import email_split_and_format, formataddr, mute_logger
...
class TestMailgateway(TestMailCommon):
    ...
    @mute_logger('galex.addons.mail.models.mail_thread', 'galex.models')
    ...
    @mute_logger('galex.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not galexbot: added as follower """
        ...
        galexbot = self.env.ref('base.partner_root')
        galexbot.active = True
        galexbot.email = 'galexbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, galexbot.email_formatted, 'groups@test.com',
                subject='GalexERPbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, galexbot)
        self.assertEqual(record4.message_ids[0].email_from, galexbot.email_formatted)
    ...
    @mute_logger('galex.addons.mail.models.mail_thread', 'galex.models.unlink', 'galex.addons.mail.models.mail_mail')
    ...
    @mute_logger('galex.addons.mail.models.mail_thread', 'galex.addons.mail.models.mail_mail', 'galex.models.unlink')
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by galex should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        galex should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since GalexERP saas-3 """
        ...
        # GalexERP 10 update: compat mode has been removed and should not work anymore

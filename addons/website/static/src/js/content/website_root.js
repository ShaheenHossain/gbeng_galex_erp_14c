galex.define('website.root', function (require) {
'use strict';
...
var WebsiteRoot = publicRootData.PublicRoot.extend(KeyboardNavigationMixin, {
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoomGalexERP();
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.galex_gmap_api_post_load = (async function galex_gmap_api_post_load() {
                ...
                await ajax.loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=galex_gmap_api_post_load&key=${key}`);
            });
        }

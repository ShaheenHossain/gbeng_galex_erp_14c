...
from galex.addons.payment.models.payment_acquirer import ValidationError
from galex.addons.payment_ingenico.controllers.main import OgoneController
from galex.addons.payment_ingenico.data import ogone
from galex.http import request
from galex.tools import DEFAULT_SERVER_DATE_FORMAT, ustr
from galex.tools.float_utils import float_compare, float_repr, float_round
...
class PaymentAcquirerOgone(models.Model):
    ...
    def _ogone_generate_shasign(self, inout, values):
        ...
        :param string inout: 'in' (galex contacting ogone) or 'out' (ogone
                             contacting galex). In this last case only some
    ...
    def ogone_form_generate_values(self, values):
        ...
        if self.save_token in ['ask', 'always']:
            temp_ogone_tx_values.update({
                'ALIAS': 'GALEX-NEW-ALIAS-%s' % time.time(),    # something unique,
...
class PaymentTxOgone(models.Model):
    ...
    def ogone_s2s_do_transaction(self, **kwargs):
        ...
        reference = self.reference or "GALEX-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), self.partner_id.id)
    ...
    def _ogone_s2s_get_tx_status(self):
        account = self.acquirer_id
        #reference = tx.reference or "GALEX-%s-%s" % (datetime.datetime.now().strftime('%Y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentToken(models.Model):
    ...
    def ogone_create(self, values):
        if values.get('cc_number'):
            ...
            alias = 'GALEX-NEW-ALIAS-%s' % time.time()

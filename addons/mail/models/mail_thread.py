...
from galex import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID
from galex.exceptions import MissingError
from galex.osv import expression

from galex.tools import ustr
from galex.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not galexbot
    ...
    def message_post(self, *,
        ...
        if values['author_id'] and values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(values['author_id']).active:  # we dont want to add galexbot/inactive as a follower
    ...
    def message_post_with_view(self, views_or_xmlid, **kwargs):
        ...
        try:
            from galex.addons.http_routing.models.ir_http import slug
            values['slug'] = slug
        except ImportError:
    ...
    def _notify_record_by_email(self, message, recipients_data, msg_vals=False,
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in GalexERP)
    ...
    def _message_auto_subscribe_followers(self, updated_values, default_subtype_ids):
        ...
        with track_visibility set. Since GalexERP v7 it is considered as being

...
from galex.exceptions import ValidationError, UserError
from galex.tools import remove_accents, is_html_empty
...
class Alias(models.Model):
    """A Mail Alias is a mapping of an email address with a given GalexERP Document
       model. It is used by GalexERP's mail gateway when processing incoming emails
       ...
       created, it becomes immediately usable and GalexERP will accept email for it.
     """
    _name = 'mail.alias'
    ...
    alias_name = fields.Char('Alias Name', copy=False, help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.galex.com>")
    alias_model_id = fields.Many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                     help="The model (GalexERP Document Kind) to which this alias "
    ...
    def name_get(self):
        """Return the mail alias display alias_name, including the implicit
           ...
           e.g. `jobs@mail.galex.com` or `jobs` or 'New Alias'
        """
        res = []
        for record in self:

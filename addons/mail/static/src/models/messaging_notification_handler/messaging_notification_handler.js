galex.define('mail/static/src/models/messaging_notification_handler/messaging_notification_handler.js', function (require) {
'use strict';
...
function factory(dependencies) {
    ...
    class MessagingNotificationHandler extends dependencies['mail.model'] {
        ...
        async _handleNotificationChannelMessage(channelId, messageData) {
            ...
            // GalexERP for users with notification "Handled by Email".
            ...
            // Chat from GalexERPBot is considered disturbing and should only be
            // shown on the menu, but no notification and no thread open.
            const isChatWithGalexERPBot = (
                channel.correspondent &&
                channel.correspondent === this.env.messaging.partnerRoot
            );
            if (!isChatWithGalexERPBot) {
                const isGalexERPFocused = this.env.services['bus_service'].isGalexERPFocused();
                // Notify if out of focus
                if (!isGalexERPFocused && channel.isChatChannel) {

...
GalexERP Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (GalexERP
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the GalexERP Web 7
...
The primary component of search views, as with all other GalexERP
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from GalexERP field types (and widgets) to search view
...
GalexERP this is done via :ref:`domains <openerpserver:domains>`. On
the other hand, the GalexERP Web 7 search view's state is modelled
...
:js:class:`~openerp.web.search.Facet` objects to GalexERP search data.
...
of the peculiarities pertaining to GalexERP's handling of fields in
...
GalexERP Web 6.1 to GalexERP Web 7.

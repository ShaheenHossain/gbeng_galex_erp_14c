...
A very basic GalexERP module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An GalexERP
...
.. note::
    ...
    restart the GalexERP server itself for it to be taken in account.
...
provides: an instance of the GalexERP Web client, with the contents of
...
will require restarting the GalexERP Server to see the changes, as
usual):
...
.. note::
    ...
    will apply only within the confines of the GalexERP Web client) and
...
GalexERP Web's template language is :doc:`qweb`. Although any
...
integration to GalexERP Web widgets.
...
To prevent this irremediable loss, we can use GalexERP's support for
...
basic GalexERP model in which our data will be stored:
...
4. And finally — after restarting GalexERP — navigating to the test

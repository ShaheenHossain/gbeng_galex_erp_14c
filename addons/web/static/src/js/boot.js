...
(function () {
    ...
    galex.testing = typeof QUnit === 'object';
    galex.remainingJobs = jobs;
    galex.__DEBUG__ = {
    ...
    galex.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__galex_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: galex.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = galex.__DEBUG__.getDependencies(job.name);
            ...
            missing = galex.__DEBUG__.getMissingJobs();
            failed = galex.__DEBUG__.getFailedJobs();
            ...
            if (galex.debug || failed.length || unloaded.length) {
                ...
                if (galex.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        galex.__DEBUG__.jsModules = {
            missing: missing,
    ...
    galex.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            galex.processJobs(jobs, services);

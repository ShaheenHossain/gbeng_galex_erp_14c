galex.define('web.mixins', function (require) {
"use strict";
...
function GalexERPEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
GalexERPEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

GalexERPEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * GalexERP Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new GalexERPEvent(this, name, info);

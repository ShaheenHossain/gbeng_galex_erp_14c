galex.define('web.ErrorDialogRegistry', function (require) {
"use strict";
...
galex.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    init: function () {
        ...
        this.galexExceptionTitleMap = {
            'galex.addons.base.models.ir_mail_server.MailDeliveryException': _lt("MailDeliveryException"),
            'galex.exceptions.AccessDenied': _lt("Access Denied"),
            'galex.exceptions.AccessError': _lt("Access Error"),
            'galex.exceptions.MissingError': _lt("Missing Record"),
            'galex.exceptions.UserError': _lt("User Error"),
            'galex.exceptions.ValidationError': _lt("Validation Error"),
            'galex.exceptions.Warning': _lt("Warning"),
        };
    ...
    rpc_error: function(error) {
        ...
        // GalexERP custom exception: UserError, AccessError, ...
        if (_.has(this.galexExceptionTitleMap, error.data.name)) {
            error = _.extend({}, error, {
                data: _.extend({}, error.data, {
                    message: error.data.arguments[0],
                    title: this.galexExceptionTitleMap[error.data.name],
                }),
            });
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("GalexERP Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("GalexERP Warning"),
            buttons: [
...
core.crash_registry.add('galex.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("GalexERP Session Expired"),
                message: _t("Your GalexERP session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
...
core.crash_registry.add('galex.http.SessionExpiredException', session_expired);

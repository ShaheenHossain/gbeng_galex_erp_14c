...
import galex
import galex.modules.registry
from galex.api import call_kw, Environment
from galex.modules import get_module_path, get_resource_path
from galex.tools import image_process, topological_sort, html_escape, pycompat, ustr, apply_inheritance_specs, lazy_property
from galex.tools.mimetypes import guess_mimetype
from galex.tools.translate import _
from galex.tools.misc import str2bool, xlsxwriter, file_open
from galex.tools.safe_eval import safe_eval, time
from galex import http, tools
from galex.http import content_disposition, dispatch_rpc, request, serialize_exception as _serialize_exception, Response
from galex.exceptions import AccessError, UserError, AccessDenied
from galex.models import check_method_name
from galex.service import db, security
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('galex.addons.web', "views")
    ...
    try:
        ...
        with registry.cursor() as cr:
            return module_installed(
                environment=Environment(cr, galex.SUPERUSER_ID, {}))
    except Exception:
        pass
    return {}

def module_boot(db=None):
    server_wide_modules = galex.conf.server_wide_modules or []
    serverside = ['base', 'web']
    dbside = []
...
def manifest_list(extension, mods=None, db=None, debug=None):
    ...
    if debug is not None:
        _logger.warning("galex.addons.web.main.manifest_list(): debug parameter is deprecated")
    mods = mods.split(',')
...
def fix_view_modes(action):
    """ For historical reasons, GalexERP has weird dealings in relation to
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if not request.uid:
            request.uid = galex.SUPERUSER_ID

        values = {k: v for k, v in request.params.items() if k in SIGN_UP_REQUEST_PARAMS}
        try:
            values['databases'] = http.db_list()
        except galex.exceptions.AccessDenied:
        ...
        if request.httprequest.method == 'POST':
            ...
            except galex.exceptions.AccessDenied as e:
                request.uid = old_uid
                if e.args == galex.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = galex.SUPERUSER_ID
...
class WebClient(http.Controller):
    ...
    def version_info(self):
        return galex.service.common.exp_version()
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = galex.tools.config.verify_admin_password('admin')
        d['list_db'] = galex.tools.config['list_db']
        d['langs'] = galex.service.db.exp_list_lang()
        d['countries'] = galex.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        d['databases'] = []
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = galex.service.db.list_db_incompatible(d['databases'])
        except galex.exceptions.AccessDenied:
            monodb = db_monodb()
            if monodb:
                d['databases'] = [monodb]
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = galex.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format = 'zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            galex.service.db.check_super(master_pwd)
            ...
            dump_stream = galex.service.db.dump_db(name, None, backup_format)
...
class Session(http.Controller):
    ...
    def modules(self):
        ...
        return module_installed(environment=request.env(user=galex.SUPERUSER_ID))
    ...
    def account(self):
        ...
        return 'https://accounts.galex.com/oauth2/auth?' + url_encode(params)
    ...
    def _content_image_get_response(
        ...
        if not image_base64:
            ...
            if not (width or height):
                width, height = galex.tools.image_guess_size_from_field_name(field)
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        if not dbname:
            ...
            try:
                ...
                registry = galex.modules.registry.Registry(dbname)
                with registry.cursor() as cr:
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: galex.tools.ustr(field[1].get('string', '').lower()))
...
class ExportFormat(object):
    ...
    def from_data(self, fields, rows):
        """ Conversion method from GalexERP's export data to whatever the

...
from galex.tools import date_utils

from galex.addons.adyen_platforms.util import AdyenProxyAuth
...
class AdyenAccount(models.Model):
    ...
    def create(self, values):
        ...
        # Create account on galex.com, proxy and Adyen
        response = adyen_account_id._adyen_rpc('create_account_holder', adyen_account_id._format_data())

        # Save adyen_uuid and proxy_token, that have been generated by galex.com and the proxy
    ...
    def action_create_redirect(self):
        ...
        The action will redirect the user to accounts.galex.com to link an GalexERP user_id to the Adyen
        account. After logging in on galex.com the user will be redirected to his DB with a token in
    ...
    def _update_kyc_status(self, checks):
        ...
        if kyc_status_message.decode() != self.kyc_status_message:
            self.sudo().message_post(body = kyc_status_message, subtype_xmlid="mail.mt_comment") # Message from GalexERP Bot

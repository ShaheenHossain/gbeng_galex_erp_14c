...
import galex
from galex import api, models, registry, exceptions, tools, http
from galex.addons.base.models.ir_http import RequestUID, ModelConverter
from galex.addons.base.models.qweb import QWebException
from galex.http import request
from galex.osv import expression
from galex.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if galex._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            galex._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if galex._geoip_resolver and request.httprequest.remote_addr:
                record = galex._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except galex.exceptions.MissingError:

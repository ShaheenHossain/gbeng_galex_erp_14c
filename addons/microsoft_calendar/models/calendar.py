...
from galex.exceptions import UserError, ValidationError
from galex.tools import html2plaintext, is_html_empty, email_normalize
...
class Meeting(models.Model):
    ...
    @api.model
    def _microsoft_to_galex_values(self, microsoft_event, default_reminders=(), default_values=None, with_ids=False):
        ...
        commands_attendee, commands_partner = self._galex_attendee_commands_m(microsoft_event)
        ...
        alarm_commands = self._galex_reminders_commands_m(microsoft_event)
        if alarm_commands:
    ...
    @api.model
    def _microsoft_to_galex_recurrence_values(self, microsoft_event, default_values=None):
        timeZone_start = pytz.timezone(microsoft_event.start.get('timeZone'))
        timeZone_stop = pytz.timezone(microsoft_event.end.get('timeZone'))
        start = parse(microsoft_event.start.get('dateTime')).astimezone(timeZone_start).replace(tzinfo=None)
        if microsoft_event.isAllDay:
    ...
    @api.model
    def _galex_attendee_commands_m(self, microsoft_event):
        commands_attendee = []
        commands_partner = []
        ...
        if microsoft_event.match_with_galex_events(self.env):
            existing_attendees = self.env['calendar.attendee'].search([
                ('event_id', '=', microsoft_event.galex_id(self.env)),
                ('email', 'in', emails)])
        ...
        for galex_attendee in attendees_by_emails.values():
            # Remove old attendees
            if galex_attendee.email not in emails:
                commands_attendee += [(2, galex_attendee.id)]
                commands_partner += [(3, galex_attendee.partner_id.id)]
        return commands_attendee, commands_partner

    @api.model
    def _galex_reminders_commands_m(self, microsoft_event):
        reminders_commands = []
        if microsoft_event.isReminderOn:
            event_id = self.browse(microsoft_event.galex_id(self.env))
    ...
    def _ensure_attendees_have_email(self):
        ...
        if invalid_event_ids:
            ...
            raise ValidationError(_("For a correct synchronization between GalexERP and Outlook Calendar, "
    ...
    def _cancel_microsoft(self):
        ...
        There are 2 cases:
          1) the organizer is an GalexERP user: he's the only one able to delete the GalexERP event. Attendees can just decline.
          2) the organizer is NOT an GalexERP user: any attendee should remove the GalexERP event.
        """
        user = self.env.user

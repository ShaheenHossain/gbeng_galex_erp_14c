...
import logging
import requests
from galex.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from galex.addons.microsoft_account.models.microsoft_service import MICROSOFT_TOKEN_ENDPOINT
from galex.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> GalexERP
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2galex(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # GalexERP -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_galex2microsoft()
        ...
        (events - synced_events)._sync_galex2microsoft()

...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by GalexERP recordset, one instance can be a single Microsoft event or a
    ...
    def galex_id(self, env):
        return self._galex_id

    def _meta_galex_id(self, microsoft_guid):
        """Returns the GalexERP id stored in the Microsoft Event metadata.
    ...
    def galex_ids(self):
        """
        Get the list of GalexERP event ids already mapped with Outlook events (self)
        """
        return tuple(e._galex_id for e in self if e._galex_id)

    def _load_galex_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing GalexERP events:
        1) extract unmapped events only,
        2) match GalexERP events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._galex_id]
        ...
        galex_events = model_env.with_context(active_test=False).search([
            '|',
            ('ms_universal_event_id', "in", unmapped_events.uids),
            ('ms_organizer_event_id', "in", unmapped_events.ids)
        ]).with_env(env)

        # 1. try to match unmapped events with GalexERP events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        galex_events_with_uids = galex_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in galex_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            galex_id = mapping.get(ms_event.iCalUId)
            if galex_id:
                ms_event._events[ms_event.id]['_galex_id'] = galex_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with GalexERP events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in galex_events}

        for ms_event in unmapped_events:
            galex_event = mapping.get(ms_event.id)
            if galex_event:
                ms_event._events[ms_event.id]['_galex_id'] = galex_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the GalexERP event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current GalexERP user is the organizer of the event according to Outlook event, so return his id.
        2) the current GalexERP user is NOT the organizer and:
           2.1) we are able to find a GalexERP user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a GalexERP user matching the organizer email address and we return False, meaning
                that no GalexERP user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        if self.organizer.get('emailAddress') and self.organizer.get('emailAddress').get('address'):
            # Warning: In Microsoft: 1 email = 1 user; but in GalexERP several users might have the same email
    ...
    def match_with_galex_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing GalexERP events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_galex_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_galex_ids_from_db(env)

...
from galex.tests.common import HttpCase

from galex.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a GalexERP event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for GalexERP events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to GalexERP events
        # -----------------------------------------------------------------------------------------

        self.expected_galex_event_from_outlook = {
            "name": "simple_event",
            "description": "my simple event",
            "active": True,
            "start": self.start_date,
        ...
        self.expected_galex_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_galex_recurrency_events_from_outlook = [
            {
    ...
    def assert_galex_event(self, galex_event, expected_values):
        """
        Assert that an GalexERP event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        galex_event_values = galex_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(galex_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, galex_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_galex_recurrence(self, galex_recurrence, expected_values):
        """
        Assert that an GalexERP recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        galex_recurrence_values = galex_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, galex_recurrence_values.get(k), msg=f"'{k}' mismatch")

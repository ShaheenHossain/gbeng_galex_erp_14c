...
from galex.exceptions import AccessDenied, AccessError
from galex.http import _request_stack

import galex.tools
from galex.tests import common
from galex.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, galex.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {

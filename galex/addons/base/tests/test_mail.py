...
from galex.tests.common import BaseCase, SavepointCase, TransactionCase
from galex.tools import (
...
class EmailConfigCase(SavepointCase):
    @patch.dict("galex.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'galex.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@galex.example.com')
        set_param('mail.catchall.domain', 'galex.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@galex.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@galex.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@galex.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@galex.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@galex.example.com>')
        set_param('mail.catchall.domain', 'galex.example.com')

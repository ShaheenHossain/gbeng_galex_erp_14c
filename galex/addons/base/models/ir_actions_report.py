...
from galex.exceptions import UserError, AccessError
from galex.tools.safe_eval import safe_eval, time
from galex.tools.misc import find_in_path
from galex.tools import config
from galex.sql_db import TestCursor
from galex.http import request
from galex.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of GalexERP so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at GalexERP start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start GalexERP with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "GalexERP is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"

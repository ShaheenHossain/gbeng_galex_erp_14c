...
from galex import api, fields, models, tools, SUPERUSER_ID, _
from galex.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from galex.exceptions import AccessDenied, AccessError, UserError, ValidationError
from galex.http import request
from galex.modules.module import get_module_resource
from galex.osv import expression
from galex.service.db import check_super
from galex.tools import partition, collections, frozendict, lazy_property, image_process
...
class Users(models.Model):
    """ User class. A res.users record models an GalexERP user and is different
        from an employee.
    ...
    def _get_default_image(self):
        """ Get a default image when the user is created without image
            ...
            https://github.com/galex/galex/blob/11.0/galex/addons/base/res/res_partner.py
        """
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by GalexERP (updates, module installation, ...)'))
        self.clear_caches()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: galex.exceptions.AccessDenied when old password is wrong
        :raise: galex.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        if not new_passwd:
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your GalexERP is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "GalexERP in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.galex.com/documentation/14.0/administration/deployment/deploy.html#https for details.",
                    source
                )

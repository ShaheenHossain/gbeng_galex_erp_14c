...
import galex
import galex.tools as tools
import galex.release as release
from galex.tools import pycompat
...
# ad_paths is a deprecated alias, please use galex.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"galex.modules.module.ad_paths" is a deprecated proxy to '
        '"galex.addons.__path__".', DeprecationWarning, stacklevel=2)
    return galex.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "galex.addons".',
    ...
    def load_module(self, name):
        ...
        galex_name = re.sub(r'^openerp.addons.(\w+)$', r'galex.addons.\g<1>', name)

        galex_module = sys.modules.get(galex_name)
        if not galex_module:
            galex_module = importlib.import_module(galex_name)

        sys.modules[name] = galex_module

        return galex_module

class GalexERPHook(object):
    """ Makes galex package also available as openerp """
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to galex.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'galex\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `galex.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^galex.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("galex.addons.base.maintenance.migrations", "galex.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``galex.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook galex.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in galex.addons.__path__:
        galex.addons.__path__.append(dd)

    # hook galex.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in galex.addons.__path__:
            galex.addons.__path__.append(ad)

    # hook galex.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in galex.addons.__path__ and os.path.isdir(base_path):
        galex.addons.__path__.append(base_path)

    # hook galex.upgrade on upgrade-path
    from galex import upgrade
    ...
    # create decrecated module alias from galex.addons.base.maintenance.migrations to galex.upgrade
    spec = importlib.machinery.ModuleSpec("galex.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["galex.addons.base.maintenance"] = maintenance_pkg
    sys.modules["galex.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to galex and "crm"-like to galex.addons
    if not getattr(initialize_sys_path, 'called', False): # only initialize once
        sys.meta_path.insert(0, UpgradeHook())
        sys.meta_path.insert(0, GalexERPHook())
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in galex.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = galex.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in galex.addons.__path__:
...
def load_openerp_module(module_name):
    """ Load an GalexERP module, if not already loaded.
    ...
    try:
        __import__('galex.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['galex.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in galex.addons.__path__:

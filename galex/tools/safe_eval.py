...
GalexERP domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an GalexERP domain expression from an untrusted source.
    ...
    except galex.exceptions.UserError:
        raise
    except galex.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except galex.http.AuthenticationError:
        raise
...
`galex.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `galex.tools.safe_eval`.
""")
    return d

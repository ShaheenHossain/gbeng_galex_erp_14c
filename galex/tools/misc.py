...
"""
Miscellaneous tools used by GalexERP.
"""
import cProfile
import collections
import datetime
...
from galex.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if galex.tools.config['db_host']:
        env['PGHOST'] = galex.tools.config['db_host']
    if galex.tools.config['db_port']:
        env['PGPORT'] = str(galex.tools.config['db_port'])
    if galex.tools.config['db_user']:
        env['PGUSER'] = galex.tools.config['db_user']
    if galex.tools.config['db_password']:
        env['PGPASSWORD'] = galex.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False, filter_ext=None):
    """Open a file from the GalexERP root, using a subdir folder.
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the GalexERP root: skip zipfile lookup.
...
def _fileopen(path, mode, basedir, pathinfo, basename=None, filter_ext=None):
    ...
    paths = galex.addons.__path__ + [config['root_path']]
...
def scan_languages():
    """ Returns all languages supported by GalexERP for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = galex.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating GalexERP domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('galex.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('galex.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = galex.fields.Datetime.from_string(value)
            value = galex.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = galex.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = galex.fields.Datetime.from_string(value)
    else:
        timestamp = value

...
class galex_resolver(etree.Resolver):
    """GalexERP specific file resolver that can be added to the XML Parser.
...
def _check_with_xsd(tree_or_str, stream, env=None):
    ...
    :param env (galex.api.Environment): If it is given, it enables resolving the
    ...
    if env:
        parser.resolvers.add(galex_resolver(env))

...
# *broken*
NO_POSTMORTEM = (galex.exceptions.AccessDenied,
                 galex.exceptions.UserError,
                 galex.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                galex.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = galex.service.common.dispatch
        elif service_name == 'db':
            dispatch = galex.service.db.dispatch
        elif service_name == 'object':
            dispatch = galex.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                galex.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                galex.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(galex.tools.exception_to_unicode(e))
        galex.tools.debugger.post_mortem(galex.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all GalexERP Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in galex.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~galex.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~galex.api.Environment` bound to current request. """
        if self._env is None:
            self._env = galex.api.Environment(self.cr, self.uid, self.context)
        return self._env

    @lazy_property
    def session(self):
        """ :class:`GalexERPSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            galex.tools.debugger.post_mortem(
                galex.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        GalexERP implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <galex.http.WebRequest.csrf_token`, the
          :data:`~galex.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          GalexERP) as e.g. it is a REST API or a `webhook
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (galex.exceptions.UserError,
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "GalexERP Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "GalexERP Session Expired"
...
GalexERP URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.galex.com/documentation/14.0/developer/reference/http.html#csrf for
more details.

* if this endpoint is accessed through GalexERP via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["galex", "addons"]:
            module = ""
        else:
...
class EndPoint(object):
    ...
    # When GalexERP clears the routing map, new EndPoint objects are created,
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('galex.addons.') and c.__module__.split(".")[2] in modules
...
class GalexERPSession(sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(GalexERPSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = galex.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = galex.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ GalexERP provides languages which may not make sense and/or may not
...
GALEX_DISABLE_SESSION_GC = str2bool(os.environ.get('GALEX_DISABLE_SESSION_GC', '0'))

if GALEX_DISABLE_SESSION_GC:
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or galex.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the GalexERP Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = galex.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        if GALEX_DISABLE_SESSION_GC:
            _logger.info('Default session GC disabled, manual GC required.')
        return sessions.FilesystemSessionStore(
            path, session_class=GalexERPSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        routing_map = werkzeug.routing.Map(strict_slashes=False, converters=None)
        for url, endpoint, routing in galex.http._generate_routing_rules([''] + galex.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in galex.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        galex.registry(db).check_signaling()
                        with galex.tools.mute_logger('galex.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = galex.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if galex.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = galex.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif galex.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, GalexERP will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in galex.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = galex.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = galex.tools.ustr(filename)

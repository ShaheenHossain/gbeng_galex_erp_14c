...
"""
GalexERP - Server
GalexERP is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - GalexERP
"""
...
__author__ = galex.release.author
__version__ = galex.release.version

# Also use the `galex` logger for the main script.
_logger = logging.getLogger('galex')
...
def report_configuration():
    ...
    """
    config = galex.tools.config
    _logger.info("GalexERP version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', galex.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = galex.tools.config
    if not galex.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = galex.modules.registry.Registry.new(dbname)
        with galex.api.Environment.manage():
            with registry.cursor() as cr:
                galex.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with galex.api.Environment.manage():
        with registry.cursor() as cr:
            galex.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    galex.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                galex.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except galex.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        galex.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = galex.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the galex server (default command)"""
    def run(self, args):
        main(args)

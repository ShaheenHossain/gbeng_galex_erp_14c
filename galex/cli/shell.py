...
class Shell(Command):
    """Start galex in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        galex.cli.server.report_configuration()
        galex.service.server.start(preload=[], stop=True)
        ...
        with galex.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = galex.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = galex.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env

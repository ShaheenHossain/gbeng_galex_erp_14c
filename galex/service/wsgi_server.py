...
# User code must use the exceptions defined in ``galex.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, galex.exceptions.RedirectWarning):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, galex.exceptions.AccessError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, galex.exceptions.AccessDenied):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, galex.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = galex.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, galex.exceptions.MissingError):
    ...
    #InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(galex.tools.exception_to_unicode(e), formatted_info)
...
def application_unproxied(environ, start_response):
    ...
    # web.session.GalexERPSession.send() and at RPC dispatch in
    # galex.service.web_services.objects_proxy.dispatch().
    ...
    with galex.api.Environment.manage():
        result = galex.http.root(environ, start_response)

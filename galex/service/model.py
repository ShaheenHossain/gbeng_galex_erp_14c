...
from galex.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = galex.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(galex.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if galex.registry(dbname)._init and not galex.tools.config['test_enable']:
                    raise galex.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    galex.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = galex.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = galex.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with galex.registry(db).cursor() as cr:

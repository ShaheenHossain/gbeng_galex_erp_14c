...
import galex
from galex import SUPERUSER_ID
from galex.exceptions import AccessDenied
import galex.release
import galex.sql_db
import galex.tools
from galex.sql_db import db_connect
from galex.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not galex.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and galex.tools.config.verify_admin_password(passwd):
        return True
    raise galex.exceptions.AccessDenied()

# This should be moved to galex.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = galex.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            galex.modules.db.initialize(cr)
            galex.tools.config['load_language'] = lang
            cr.commit()

        registry = galex.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = galex.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = galex.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = galex.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = galex.tools.config['db_template']
    ...
    if galex.tools.config['unaccent']:
        try:
            db = galex.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    galex.sql_db.close_db(db_original_name)
    db = galex.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = galex.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = galex.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = galex.tools.config.filestore(db_original_name)
    to_fs = galex.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    galex.modules.registry.Registry.delete(db_name)
    galex.sql_db.close_db(db_name)
    ...
    fs = galex.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'galex_dump': '1',
        'db_name': cr.dbname,
        'version': galex.release.version,
        'version_info': galex.release.version_info,
        'major_version': galex.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = galex.tools.config.filestore(db_name)
            ...
            galex.tools.exec_pg_command(*cmd)
            if stream:
                galex.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                galex.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = galex.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if galex.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = galex.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = galex.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    galex.modules.registry.Registry.delete(old_name)
    galex.sql_db.close_db(old_name)
    ...
    old_fs = galex.tools.config.filestore(old_name)
    new_fs = galex.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    galex.tools.config.set_admin_password(new_password)
    galex.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        galex.tools.config['update']['base'] = True
        galex.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@galex.tools.mute_logger('galex.sql_db')
...
def list_dbs(force=False):
    if not galex.tools.config['list_db'] and not force:
        raise galex.exceptions.AccessDenied()

    if not galex.tools.config['dbfilter'] and galex.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, GalexERP will not
        ...
        res = sorted(db.strip() for db in galex.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [galex.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of GalexERP
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if galex.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        galex.sql_db.close_db(database_name)
...
def exp_list_lang():
    return galex.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(galex.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()

...
import galex
from galex.modules import get_modules
from galex.modules.registry import Registry
from galex.release import nt_service_name
from galex.tools import config
from galex.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'galex.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The GALEX_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("GALEX_MAX_HTTP_THREADS")
    ...
    # is included in all Python 3 releases installed on all operating systems supported by GalexERP.
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(galex, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in galex.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="galex.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from galex.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = galex.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(galex.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="galex.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="galex.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = galex.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('galex: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(galex.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The GALEX_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("GALEX_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = galex.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('galex.netsvc.rpc.request')
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(galex.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            galex.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if galex.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    from galex.tests.common import GalexERPSuite
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                t0 = time.time()
                t0_sql = galex.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             galex.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the galex http server and cron processor.
    """
    global server
    ...
    if galex.evented:
        server = GeventServer(galex.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(galex.service.wsgi_server.application)
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which GalexERP is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(galex.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not galex.evented:
        if inotify:
    ...
    if getattr(galex, 'phoenix', False):
        _reexec()

# -*- coding: utf-8 -*-
"""
The module :mod:`galex.tests.common` provides unittest test cases and a few
...
import galex
from galex import api
from galex.models import BaseModel
from galex.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from galex.service import security
from galex.sql_db import Cursor
from galex.tools import float_compare, single_email_re
from galex.tools.misc import find_in_path
from galex.tools.safe_eval import safe_eval
...
# The galex library is supposed already configured.
ADDONS_PATH = galex.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = galex.SUPERUSER_ID


def get_db_name():
    db = galex.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding GalexERP module name.
    """
    def register(func):
        # register func by galex module name
        if func.__module__.startswith('galex.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most galex operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class GalexERPSuite(unittest.suite.TestSuite):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('galex.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common GalexERP-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.galex.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~galex.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('galex.sql_db.Cursor.execute', execute):
            with patch('galex.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/galex/addons/" in filename:
                        filename = filename.rsplit("/galex/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = galex.registry(get_db_name())
        ...
        #: :class:`~galex.api.Environment` for the current test case
        self.env = api.Environment(self.cr, galex.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = galex.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, galex.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_galex')
        self.chrome_pid = None

        otc = galex.tools.config
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way GalexERP widgets are created they all appear as Class(...)
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, galex.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, galex.tools.config['http_port'], url)
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('galex.service.http.request.')]
        ...
        if request_threads:
            ...
            galex.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        galex.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            galex.http.root.session_store.delete(self.session)

        self.session = session  = galex.http.root.session_store.new()
        session.db = db
        ...
        galex.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, galex.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "galex.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "galex.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~galex.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~galex.fields.Many2many` fields, the field itself is a
    :class:`~galex.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~galex.fields.One2many` are reified as
    :class:`~galex.tests.common.O2MProxy`.

    Because the :class:`~galex.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~galex.tests.common.O2MProxy.new` and
    :meth:`~galex.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: galex.models.Model
    ...
    :type view: int | str | galex.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~galex.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~galex.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = galex.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = galex.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return galex.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return galex.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from galex.tests.common have a test_tags

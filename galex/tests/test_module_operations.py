...
import galex
from galex.tools import config, topological_sort, unique
from galex.netsvc import init_logger
from galex.tests import standalone_tests
import galex.tests.loader

_logger = logging.getLogger('galex.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with galex.api.Environment.manage():
        with galex.registry(db_name).cursor() as cr:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of GalexERP modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra GalexERP modules")
...
def test_full(args):
    ...
    with galex.api.Environment.manage():
        with galex.registry(args.database).cursor() as cr:
            env = galex.api.Environment(cr, galex.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = galex.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        galex.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        galex.tools.config['addons_path'] = ','.join([args.addons_path, galex.tools.config['addons_path']])
        if args.data_dir:
            galex.tools.config['data_dir'] = args.data_dir
        galex.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('galex.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('galex.sql_db').setLevel(logging.CRITICAL)

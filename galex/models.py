...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            assert self.__module__.startswith('galex.addons.'), \
                "Invalid import of %s.%s, it should start with 'galex.addons'." % (self.__module__, name)
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """Base class for GalexERP models.

    GalexERP models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~galex.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~galex.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~galex.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~galex.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = galex.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or galex.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], galex.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], galex.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from galex.fields import Char, Integer
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~galex.addons.base.models.res_company`).

        For :class:`~galex.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~galex.fields.Integer`,
          :class:`~galex.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~galex.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~galex.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~galex.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~galex.fields.Date` and
              :class:`~galex.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`galex.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`galex.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~galex.fields.One2many` and
          :class:`~galex.fields.Many2many` use a special "commands" format to
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an GalexERP domain.
    ...
    def ensure_one(self):
        ...
        :raise galex.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~galex.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~galex.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/galex/master-nochange-fp/galex/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`galex.tools.populate` for population tools and applications.
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted GalexERP models.

    GalexERP models are created by inheriting from this class::

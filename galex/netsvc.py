...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('galex.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='galex')
    ...
    # Check that handler.stream has a fileno() method: when running GalexERP
...
DEFAULT_LOG_CONFIGURATION = [
    'galex.http.rpc.request:INFO',
    'galex.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['galex:DEBUG', 'galex.sql_db:INFO', 'galex.http.rpc:DEBUG'],
    'debug_rpc': ['galex:DEBUG', 'galex.sql_db:INFO', 'galex.http.rpc.request:DEBUG'],
    'debug': ['galex:DEBUG', 'galex.sql_db:INFO'],
    'debug_sql': ['galex.sql_db:DEBUG'],
    'info': [],
    'runbot': ['galex:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['galex:WARNING', 'werkzeug:WARNING'],
    'error': ['galex:ERROR', 'werkzeug:ERROR'],
    'critical': ['galex:CRITICAL', 'werkzeug:CRITICAL'],
}
